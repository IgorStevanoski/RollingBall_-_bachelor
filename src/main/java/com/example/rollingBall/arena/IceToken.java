...
public class IceToken extends Cylinder implements Positioned {
    private Rotate rotation;
    private Translate upndown;
    private Translate position;

    public IceToken() {
        this(new Translate(0,- (Const.PODIUM_HEIGHT / 2 + Const.ICETOKEN_RADIUS),0));
    }
    public IceToken(IceToken iceToken) {
        this(new Translate(
                iceToken.position.getX(),
                iceToken.position.getY(),
                iceToken.position.getZ()
        ));
    }
    public IceToken(Translate position) {
        super (Const.ICETOKEN_RADIUS, Const.ICETOKEN_HEIGHT );
        ...
        material.setDiffuseMap(Const.ICETOKEN_TEXTURE);
        material.setSelfIlluminationMap(Const.ICETOKEN_TEXTURE);
