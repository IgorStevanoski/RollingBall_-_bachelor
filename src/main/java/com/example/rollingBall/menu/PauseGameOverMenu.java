...
public class PauseGameOverMenu {
    ...
    private Label  labelNewRecord;
    private Button continueButton;
    private Button newGameButton;
    private Button mainMenuButton;
    private GameSubScene gameSubScene;

    public PauseGameOverMenu(GameSubScene gameSubScene) {
        this.gameSubScene = gameSubScene;

        labelTitle = new LabelC("Game over!");

        continueButton = new ButtonC("Leaderboards");
        ...
        labelNewRecord = new LabelC("");
        labelNewRecord.setTextFill(Color.LIGHTGREEN);
        labelNewRecord.getTransforms().add(
                new Translate((Main.getCurrent_window_width() + Const.MENU_PANE_WIDTH) / 2,
                        Main.getCurrent_window_height() / 2 - Const.MENU_BUTTON_HEIGHT)
        );
    ...
    public void updateLabelNewRecord() {
        labelNewRecord.setText("You have set a new record!");
    }
    ...
    private void handleButton(int code){
        if (code == 0){ // continue
            ...
            gameSubScene.handleLeaderboardsButton();
